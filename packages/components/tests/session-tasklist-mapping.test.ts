...
import {
  ...
  buildSessionTaskListTasks,
  getEffectiveLatestMessageAt,
  getEffectiveSessionActivitySummary,
} from '../src/components/sessions/session-tasklist-mapping';
...
describe('buildSessionTaskListTasks child-time aggregation', () => {
  test("parent task's latestMessageAt reflects the most recent child activity", () => {
    ...
    const tasks = buildSessionTaskListTasks(
      [parent],
      {
        scope: 'my',
  ...
  test('does not treat durable running status as working without live presence', () => {
    ...
    const tasks = buildSessionTaskListTasks([session], {
      scope: 'my',
      currentUserId: 'user-1',
      defaultTitle: 'Untitled',
    });
...
describe('buildSessionTaskListTasks repo + PR mapping', () => {
