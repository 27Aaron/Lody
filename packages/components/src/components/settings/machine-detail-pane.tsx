...
export function MachineDetailPane(props: MachineDetailPaneProps) {
  ...
  const isOnline = useMachineOnlineStatus(machine.id) === 'online';
  ...
  const shareControlVisible = !isMobile && !renaming && isOwn && !!onSharedWithTeamChange;
  const restartVisible = !isMobile && !renaming && !!onRestartDaemon;
  const revokeVisible = !isMobile && !renaming && canRevokeCredentials && !!onRevokeCredentials;
  const deleteVisible = !isMobile && !renaming && canDelete;
  const managementGroupVisible =
    shareControlVisible || restartVisible || (!isMobile && !renaming && !!onPing);
  const destructiveGroupVisible = revokeVisible || deleteVisible;
    ...
    <div className="flex h-full min-h-0 w-full min-w-0 flex-col">
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            >
              {renaming ? (
                <Input
                  ...
                  onChange={(event) => setRenameDraft(event.target.value.replace(/[\r\n]+/g, ' '))}
                  onKeyDown={(event) => {
                    if (event.key === 'Escape') {
            ...
            {deleteVisible && (
              <Button
                ...
                size="icon"
                className="h-7 w-7 shrink-0 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                aria-label={t('workspace.machines.deleteOffline', 'Delete offline machine')}
            ...
            {actionsMenuVisible && (
              <DropdownMenu>
                ...
                >
                  ...
                  {isMobile && isOwn && onSharedWithTeamChange && (
                    <DropdownMenuItem
                      onSelect={(event) => {
                        event.preventDefault();
                        if (sharing) return;
                  ...
                  {isMobile && canDelete && (
                    <>
                      ...
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                        <span>
                          {t('workspace.machines.deleteOffline', 'Delete offline machine')}
                        </span>
                      </DropdownMenuItem>
                    </>
                  )}
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-md border border-primary/25 bg-primary/5 px-3 py-2">
            <div className="min-w-0">
              ...
              <div className="font-mono text-[11px] text-muted-foreground">
                {t('settings.agent.machineLifecycle.updateVersion', 'v{{current}} -> v{{latest}}', {
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('workspace.machines.deleteConfirmTitle', 'Delete offline machine?')}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t('workspace.machines.deleteConfirmDescription', {
                machineName: machine.name,
                defaultValue:
                  'Remove {{machineName}} from this workspace until it connects again. Use this only for retired machines that will stay offline.',
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
